...
Write-Output "`nTotal VUH(Virtual User Hours) needed to perform this test will be - $VUH"
    ...
    {
        $parameters = @()
        if ($secretnew.Length -gt 0) { $parameters += "--secret $secretnew" }
        if ($certificates.Length -gt 0) { $parameters += "--certificate $certnew" }
        if ($environment.Length -gt 0) { $parameters += "--env $envnew" }
